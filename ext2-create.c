...
void write_block_bitmap(int fd)
{
	...
	int initial_blocks[] = {SUPERBLOCK_BLOCKNO, BLOCK_GROUP_DESCRIPTOR_BLOCKNO,BLOCK_BITMAP_BLOCKNO, INODE_BITMAP_BLOCKNO,INODE_TABLE_BLOCKNO, ROOT_DIR_BLOCKNO,LOST_AND_FOUND_DIR_BLOCKNO, HELLO_WORLD_FILE_BLOCKNO};
	for(int i = 0; i < sizeof(initial_blocks)/sizeof(initial_blocks[0]); i++){
		map_value[initial_blocks[i] / 8] |= (1 << (initial_blocks[i] % 8));
...
void write_inode_bitmap(int fd)
{
	...
	u8 map_value[BLOCK_SIZE];
	int initial_inodes[] = {EXT2_ROOT_INO, LOST_AND_FOUND_INO, HELLO_WORLD_INO};
    for (int i = 0; i < sizeof(initial_inodes)/sizeof(initial_inodes[0]); i++) {
        map_value[initial_inodes[i] / 8] |= (1 << (initial_inodes[i] % 8));
    }
	// TODO It's all yours
